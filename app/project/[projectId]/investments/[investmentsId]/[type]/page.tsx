...
export default function InvestmentsDetailPage() {
  ...
  const getPeriodData = () => {
    switch (investment?.period || "Порічно") {
      ...
      default:
        return [
          { name: `${investment?.year || ""} рік`, value: "0", editable: true },
          { name: "ЗА РІК", value: "0", editable: false },
        ];
    }
  };
  ...
  const [periodData, setPeriodData] = React.useState({
    ...
    period: investment?.period || "Порічно",
  ...
  const handleSave = () => {
    ...
    if (investment && type) {
      ...
      updateInvestment(investment.year, fieldToUpdate, totalValue);
    }
      ...
      <header className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold">
            Інвест {pageType} {periodData.year} року
          </h1>
